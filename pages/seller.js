  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await fetch(`/api/products?seller=${sellerId}`);
        const data = await result.json();
        dispatch({ type: 'FETCH_SUCCESS', payload: data });

        dispatch({ type: 'FETCH_SELLER_REQUEST' });
        const userResult = await fetch(`/api/users/seller/${sellerId}`);
        const userData = await userResult.json();
        dispatch({ type: 'FETCH_SELLER_SUCCESS', payload: userData });
      } catch (err) {
